...
const EventSchema = yup.object().shape({
  ...
  ticketTypes: yup.array().of(
    yup.object().shape({
      ...
      price: yup.number().min(0, "Price must be >= 0"),
      quota: yup.number().min(1, "Quota must be >= 1"),
    })
  ),
      ...
      >
        {(props: FormikProps<IEventForm>) => {
          ...
          return (
                    ...
                    <>
                      {values.ticketTypes.map((_, index) => (
                        <div key={index} className="border p-4 rounded mb-2 space-y-2">
                          ...
                          <div>
                            <label>Price</label>
                            <Field name={`ticketTypes.${index}.price`} type="number" as={Input} />
                          </div>
                          <div>
                            <label>Quota</label>
                            <Field name={`ticketTypes.${index}.quota`} type="number" as={Input} />
                          </div>
                      ...
                      <Button type="button" onClick={() => push({ name: "", price: 0, quota: 0 })}>
                        Add Ticket Type
                      </Button>
                    </>
                  )}
            ...
            {/* Promotions */}
            <div>
              <h3 className="font-semibold mt-4">Promotions</h3>
                  ...
                  <>
                    {values.promotions.map((_, index) => (
                      <div key={index} className="border p-4 rounded mb-2 space-y-2">
                        <div>
                          <label>Name</label>
                          <Field name={`promotions.${index}.name`} as={Input} />
                        </div>
                        <div>
                          <label>Discount (%)</label>
                          <Field type="number" name={`promotions.${index}.discountPercentage`} as={Input} />
                        </div>
                        <Button
                          type="button"
                          variant="destructive"
                    ...
                    <Button
                      type="button"
                      onClick={() => push({ name: "", discountPercentage: 0 })}
                    >
                      + Add Promotion
                    </Button>
                  </>
